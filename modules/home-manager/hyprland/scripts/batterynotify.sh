...
fn_notify() {
  notify-send -a "Power" $1 -u $2 -i $5 "$3" "$4" -h string:x-canonical-private-synchronous:"$6" -h boolean:SWAYNC_BYPASS_DND:true -p -e # Call the notify-send command with the provided arguments \$1 is the flags \$2 is the urgency \$3 is the title \$4 is the message \$5 is the icon
}
...
fn_percentage() {
  if [[ "$battery_percentage" -ge "$unplug_charger_threshold" ]] && [[ "$battery_status" != "Discharging" ]] && [[ "$battery_status" != "Full" ]] && (((battery_percentage - last_notified_percentage) >= $interval)); then
    ...
    fn_notify "-t 5000" "CRITICAL" "Battery Charged" "Battery is at $battery_percentage%. You can unplug the charger!" "battery-full-charged-symbolic" "battery-full"
    ...
    while [ $count -gt 0 ] && [[ $battery_status == "Discharging"* ]]; do
      ...
      fn_notify "-t 5000" "CRITICAL" "Battery Critically Low" "$battery_percentage% is critically low. Device will execute $execute_critical in $((count / 60)):$((count % 60)) ." "battery-empty-symbolic" "battery-critically-low"
      count=$((count - 1))
      sleep 1
    done
    ...
    fn_notify "-t 5000" "CRITICAL" "Battery Low" "Battery is at $battery_percentage%. Connect the charger." "battery-level-10-symbolic" "battery-low"
    last_notified_percentage=$battery_percentage
  fi
}
...
fn_status() {
  ...
  "Discharging")
    ...
    if [[ "$prev_status" != "Discharging" ]] || [[ "$prev_status" == "Full" ]]; then
      ...
      fn_notify "-t 5000" "$urgency" "Charger Plug OUT" "Battery is at $battery_percentage%." "battery-caution-symbolic" "battery-unplug"
      $execute_discharging
    fi
    fn_percentage
    ;;
  "Not"* | "Charging")
    ...
    if [[ "$prev_status" == "Discharging" ]] || [[ "$prev_status" == "Not"* ]]; then
      ...
      fn_notify "-t 5000" "$urgency" "Charger Plug In" "Battery is at $battery_percentage%." "battery-level-100-plugged-in-symbolic" "battery-plug-in"
      $execute_charging
    fi
    fn_percentage
    ;;
  "Full")
    ...
    if [[ $battery_status != "Discharging" ]]; then
      ...
      if [[ "$prev_status" == *"harging"* ]] || ((now - lt >= $((notify * 60)))); then
        fn_notify "-t 5000" "CRITICAL" "Battery Full" "Please unplug your Charger" "battery-full-symbolic" "battery-full"
